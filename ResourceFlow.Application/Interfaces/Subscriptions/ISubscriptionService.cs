...
namespace ResourceFlow.Application.Interfaces.Subscriptions
{
    ...
    {
        Task<ApiResponse<Domain.Entities.SubscriptionModels.Subscription>> CreatePlanAsync(CreateSubscriptionPlanDto dto,int userId);
        Task<ApiResponse<IEnumerable<SubscriptionResponseDto>>> GetAllPlansAsync();
        Task<ApiResponse<Domain.Entities.SubscriptionModels.Subscription>?> GetPlanByIdAsync(int id);
